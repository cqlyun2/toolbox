<template>
  <div class="space-y-6">
    <div class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-500 transition-all cursor-pointer"
      ...
      <input ref="fileInput" type="file" accept=".doc,.docx" @change="handleFile" class="hidden" />
      <svg class="w-12 h-12 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
      </svg>
      <p class="text-gray-600">点击或拖拽上传 Word 文档</p>
      <p class="text-sm text-gray-400 mt-1">支持 .doc, .docx 格式</p>
    </div>
...
const handleDrop = (e) => {
  ...
  if (f && (f.name.endsWith('.doc') || f.name.endsWith('.docx'))) file.value = f
}
...
const convert = async () => {
  ...
  try {
    const res = await axios.post('/api/pdf/word-to-pdf', formData)
